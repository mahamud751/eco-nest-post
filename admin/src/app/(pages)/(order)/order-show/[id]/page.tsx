...
import { Order, BaseEditProps, Photo } from "@/services/types";
...
import OrderShow from "@/components/organisms/OrderShow";
...
const ShowOrder: React.FC<BaseEditProps> = ({ params }) => {
  ...
  return (
    <LoadingError loading={loading} error={error}>
      <div className="flex justify-end mt-5">
        <Link href={`/order-list/${params.id}`}>
          <Button
            variant="contained"
            startIcon={<EditIcon />}
      ...
      >
        <>{data ? <OrderShow data={data} /> : <p>No data available.</p>}</>
